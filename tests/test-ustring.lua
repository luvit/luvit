...
require('tap')(function(test)
  test('ustring', function(expect)
    ...
    assert(tostring(ustring.char(0x0024)) == "$")
    assert(tostring(ustring.char(0x00A2)) == "¢")
    assert(tostring(ustring.char(0x0939)) == "ह")
    assert(tostring(ustring.char(0x20AC)) == "€")
    assert(tostring(ustring.char(0x10348)) == "𐍈")

    local u = ustring.char(0x0024, 0x00A2, 0x0939, 0x20AC, 0x10348)
    assert(tostring(u) == "$¢ह€𐍈")
    assert(u:codepoint() == 0x0024)
    assert(u:codepoint(2) == 0x00A2)
    assert(u:codepoint(-1) == 0x10348)
    local a, b, c = u:codepoint(1, 3)
    assert(a == 0x0024)
    assert(b == 0x00A2)
    assert(c == 0x0939)
    local a, b, c = u:codepoint(-3, -1)
    assert(a == 0x0939)
    assert(b == 0x20AC)
    assert(c == 0x10348)
