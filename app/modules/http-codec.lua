...
local function parser(isClient, emit)
  ...
  return function (chunk)
    ...
    if state == 0 then
      ...
      while true do
        ...
        if s == offset then
          local event = item
          -- TODO emit extra data after headers in same chunk
          item = nil
          headers = {}
          offset = 1
          head = nil
...
local function serverEncoder(write)
...
local function clientEncoder(write)
  return function (item)
    if type(item) ~= 'table' then
      return write(item)
    end
...
function exports.server(app)
  return function (emit)
    return parser(false, app(serverEncoder(emit)))
  end
end

function exports.client(app)
  return function (emit)
    return parser(true, app(clientEncoder(emit)))
  end
end
