...
## Event Loop functions
 ...
 - uv_ref:   Manually modify the event loop's reference count. Useful if the user
 - uv_unref: wants to have a handle or timeout that doesn't keep the loop alive.

 - uv_prepare_init:  Every active prepare handle gets its callback called exactly
 - uv_prepare_start: once per loop iteration, just before the system blocks to
 - uv_prepare_stop:  wait for completed i/o.

 - uv_check_init:  Every active check handle gets its callback called exactly
 - uv_check_start: once per loop iteration, just after the system returns from 
 - uv_check_stop:  blocking.
 
 - uv_idle_init:  Every active idle handle gets its callback called repeatedly 
 - uv_idle_start: until it is stopped. This happens after all other types of
 - uv_idle_stop:  callbacks are processed.  When there are multiple "idle"
...
## TTY
 ...
 - uv_guess_handle: Used to detect what type of stream should be used with a
                    given file descriptor.  For isatty() functionality use this
                    function and test for UV_TTY.
...
## Misc
 ...
 - sockaddr_in uv_ip4_addr: Convert string ip addresses to binary structures
 - sockaddr_in6 uv_ip6_addr: Convert string ip addresses to binary structures
